...
void list::remove_at(int index){
  _set_current_index(index + 1);
  list_node* tempNode = _current->prev;
...
void list::remove(const list_element& item){
  ...
  for(_current_index = 0; _current->data != item && _current_index != _size; _current_index++)
    _current = _current->next;
  if(_current_index != _size){
    list_node* tempNode = _current;
    _current = _current->next;
    tempNode->prev->next = tempNode->next;
    tempNode->next->prev = tempNode->prev;
    delete tempNode;
...
size_t list::_compute_index(int index) const{
  ...
  if(index > _size) return index % _size;
}
