...
typedef int list_element;  // change this if you want a list of 
...
struct list_node { // base class for list nodes
  ...
  list_node* next;                    
  list_node();
  // create node w/ given prev and next pointers
  list_node(list_node* p,list_node* n);  
  // or all 3.
  list_node(list_node* p, const list_element & d, list_node* n);     
};
...
private:
 ...
 // data fields:
  ...
  mutable list_node* _current;  // points to a node in the list beyond 
     //the front sentinel
